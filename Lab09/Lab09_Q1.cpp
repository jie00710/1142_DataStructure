...
private:
    ...
    TreeNode* deleteNode(TreeNode* node, int val) {
        if (node == nullptr) {
            cout << "值 " << val << " 不存在，無法刪除。" << endl;
            return nullptr; // 沒有找到要刪除的節點
        }

        if (val < node->data) {
            node->left = deleteNode(node->left, val); // 在左子樹刪除
        }
        else if (val > node->data) {
            node->right = deleteNode(node->right, val); // 在右子樹刪除
        }
        else{
            // 找到要刪除的節點
            if (node->left == nullptr) {
                ...
                TreeNode* temp = node->left; // 只有左子樹
                delete node; // 刪除當前節點
                return temp; // 回傳新的子樹根節點
            }
            else {
                // 有兩個子樹，找到右子樹的最小值替代當前節點
