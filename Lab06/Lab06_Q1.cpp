...
public:
    ...
    void enqueue(Card card) {
        ...
        Rear = static_cast<int>(deck.size()) - 1; // 更新尾端指標
    }
    // 從佇列中移除最前端元素
    void dequeue() {
        if (isEmpty()) { // 檢查是否空
    ...
    Card front() { // 取得最前端元素的值
        if (Front == -1) {
            cout << "排堆為空！" << endl;
            return Card(); // 回傳一個空的Card對象
        }
        return deck[Front]; // 回傳最前端元素的值
    }
    bool isEmpty() const {// 判斷牌堆是否為空
        return Front == -1;
    }
    bool isFull() const {// 判斷牌堆是否已滿
        return deck.size() == capacity;//當牌堆的牌等於最大容量，表示牌堆已滿，回傳True
    }
};
...
void playerTurn(Player* player, Queue& cardDeck) {// 玩家回合
    ...
    while (player->score < 21 && !cardDeck.isEmpty()) {// 玩家小於21點且牌堆不為空
        ...
        if (choice == 'h') {//如果玩家選擇抽牌
            Card newCard = cardDeck.front();//抽出牌堆中front指向的牌
            cardDeck.dequeue();//從牌堆中移除這張牌
...
void dealerTurn(Player* dealer, Queue& cardDeck) {// 莊家回合
    while (dealer->score < 17 && !cardDeck.isEmpty()) {// 莊家小於17點且牌堆不為空
        ...
        //注意:卡牌變數的類型是Card，並且使用front抽牌
            Card newCard = cardDeck.front();//抽出front指向的牌
            cardDeck.dequeue();//從牌堆移除這張牌
...
int main() {
    ...
    initializePlayer(&player, "玩家", cardDeck);//初始化玩家手上的牌
    initializePlayer(&dealer, "莊家", cardDeck);//初始化莊家手上的牌
